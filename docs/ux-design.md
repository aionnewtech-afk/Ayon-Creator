...
- **★ Vídeo gerado automaticamente (preparação Missão 9):** o widget é o player de vídeo com o MP4 já composto (§3.5). Sem **Editar** (não há texto para editar num vídeo pronto) — **Regenerar** vira **Tentar novamente**/**Gerar de novo**, que reaciona o pipeline completo do Fluxo 13 (nova cobrança de crédito `video_generation` só em caso de novo sucesso, mesmo princípio de nunca cobrar por falha). Enquanto o pipeline roda, o cartão mostra o estado "gerando vídeo" (§4.5) no lugar do player, sem bloquear a revisão das demais peças da campanha.
