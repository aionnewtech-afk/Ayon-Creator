...
| ONB-4 | Perfil da Marca | Visão persistente e editável da identidade (história, produtos, clientes, tom, concorrentes, objetivos, diferenciais, palavras proibidas/favoritas, e — ★ Missão 11 — identidade visual: logo/cores/fonte/estilo/voz, §4.12) | editando, salvo | Menu "Conheça sua Empresa" a qualquer momento | — |
...
**★ Escopo da Missão 9, Etapa 1:** o formato `video` ganha geração automática a partir de banco de vídeo licenciado (`licensed_stock_video`, [flows.md Fluxo 13](flows.md#fluxo-13--pipeline-de-geração-de-vídeo-n8n-★-novo-missão-9)) — CAMP-4/5 ganham o estado "gerando vídeo" (assíncrono, tipicamente mais lento que texto) e um estado de falha com ação "Tentar novamente". Primeira vez que uma tela do MVP precisa refletir progresso que não termina dentro do mesmo request — mecanismo exato (Realtime vs. polling) é decisão em aberto (§10). Avatar de IA (`ai_avatar`/`hybrid`) não tem tela nesta revisão — fica para o doc-first da Etapa 2.
...
| CAMP-4 | Gerando Pacote | Checklist de formatos: texto sendo gerado por IA (sequencial, Server Action síncrona), vídeo sendo gerado automaticamente (★ preparação Missão 9, assíncrono via Fluxo 13) e formatos visuais restantes aguardando upload do cliente | gerando (formato textual), **gerando vídeo** ★ novo (assíncrono, sem teto de tempo fixo), aguardando upload (formato visual), erro em um formato, **falha na geração de vídeo** ★ novo | CAMP-3 | CAMP-5 |
| CAMP-5 | Revisão e Aprovação | Revisar cada peça de texto (aprovar/editar/rejeitar/regenerar), revisar vídeo gerado automaticamente (aprovar/rejeitar/**tentar novamente**, ★ preparação Missão 9) ou enviar o arquivo de cada peça visual restante | aguardando aprovação, editando, regenerando (texto), aguardando upload (visual), **gerando vídeo** ★ novo, **falha — tentar novamente** ★ novo | CAMP-4 | CAMP-5 (loop) → CAMP-6 |
...
- **★ Missão 9, Etapa 1:** o item `vídeo` (quando `production_mode = licensed_stock_video`) mostra na fila/**gerando** (estado assíncrono, sem barra determinística de progresso — o produto não sabe de antemão quanto tempo o pipeline externo leva)/pronto/**falhou**. "Falhou" nunca é mudo — sempre acompanhado de um botão **Tentar novamente**, sem jargão técnico ("não conseguimos gerar esse vídeo agora", nunca stack trace ou código de erro do fornecedor). `ai_avatar`/`hybrid` ficam para a Etapa 2, sem tela própria ainda.
- **★ Progresso granular (Missão 11, ajuste — percentual + tempo estimado):** enquanto `vídeo`/`stories`/`carousel`/`thumbnail` (`production_mode = licensed_stock_video`/`licensed_stock_photo`) estão em `generating`, o texto genérico "gerando..." vira uma frase específica da etapa atual (`pipeline_runs.stage`, [architecture.md §14.9](architecture.md#149-progresso-granular-★-percentual--tempo-estimado)) — "Selecionando a voz da marca..." (só na 1ª geração), "Gerando narração...", "Buscando cenas...", "Renderizando...", "Aplicando identidade visual...", "Finalizando..." — sempre linguagem de negócio, nunca nome de fornecedor/API (Princípio do Consultor Permanente, §1, item 3). Acompanhada de uma barra de progresso aproximada (`progress_percent`) e, quando disponível, tempo estimado restante (`estimated_remaining_seconds`) — "quando possível": ausência de estimativa numa etapa específica não é tratada como erro, só omite o texto de tempo naquele momento.
- **★ `stories`/`carousel`/`thumbnail` (preparação Missão 11):** quando `production_mode = licensed_stock_photo`, mostram os mesmos estados do vídeo (fila/gerando/pronto/falhou), mas **sempre com a opção de upload manual disponível ao lado**, mesmo depois de uma geração automática bem-sucedida — nunca perde essa alternativa, diferente do vídeo.
...
- **★ Vídeo gerado automaticamente (Missão 9; player melhor + sem legenda, Missão 11):** o widget é o player de vídeo com o MP4 já composto (§3.5), agora maior/em destaque, **sem legenda embutida** (removida — arch. §14.2). Sem **Editar** enquanto editor de timeline (fora de escopo do MVP, PRD §9.2) — "Editar" abre o roteiro (mesmo campo de texto já usado pelos formatos textuais) para ajuste antes de gerar de novo. **Gerar de novo** reaciona o pipeline completo do Fluxo 13 (nova cobrança de crédito `video_generation` só em caso de novo sucesso, mesmo princípio de nunca cobrar por falha). Novos, ao lado dos botões existentes: **Baixar** (link direto para o MP4), **Copiar link** e **Compartilhar** (Web Share API nativa do navegador quando disponível) — arquitetura já preparada para uma futura ação "Publicar no Instagram/Facebook" sem redesenho (arch. §14.10), não implementada nesta missão. Enquanto o pipeline roda, o cartão mostra a etapa atual + percentual/tempo estimado (§4.5) no lugar do player, sem bloquear a revisão das demais peças da campanha.
- **★ `stories`/`carousel`/`thumbnail` gerados automaticamente (Missão 11, ajuste — composição real):** mesmo padrão do vídeo (estado/etapa/pronto/falhou, **Baixar**/**Copiar link**/**Compartilhar**); **Enviar arquivo** (upload manual) fica sempre visível ao lado de **Gerar automaticamente**/**Gerar de novo** — nunca some, mesmo depois de uma geração automática já aprovada (decisão do dono do produto). O preview mostra a composição final pronta — foto + branding + tipografia + título + elementos gráficos (arch. §14.4) — nunca uma fotografia crua ou um frame de vídeo sem tratamento; para `thumbnail`, o mesmo padrão com foco em leitura em miniatura (arch. §14.5).
- **★ Múltiplas opções (Missão 11, ajuste):** quando a rodada gera mais de 1 candidato (arch. §14.4.2), o cartão mostra uma grade pequena com todos os candidatos lado a lado (mesma identidade visual, foto de fundo variando) em vez de um preview único — usuário clica no candidato preferido para selecioná-lo (`content_pieces.selected_version_id`); os demais continuam visíveis, podendo ser escolhidos depois, sem gerar de novo. Quando só 1 candidato existe (custo não permitiu mais, ou o formato não usa esse mecanismo), o cartão volta ao preview único de sempre — sem UI de escolha desnecessária.
...
| Seletor de Cor (color picker) ★ novo (Missão 11) | ONB-4 — cor primária e cor secundária da marca (§4.12), input de hex/color picker padrão do navegador, sem paleta customizada |
...
### 4.12 Identidade Visual (Perfil da Marca) ★ novo (Missão 11) — ativo permanente da marca

Nova seção dentro do Perfil da Marca (ONB-4, §2 — nav) — mesmo espírito de "visão persistente e editável da identidade" já usado para história/produtos/tom de voz, agora estendido à identidade visual: definida **uma vez**, aplicada automaticamente em todo vídeo/thumbnail/stories/carrossel gerado depois (e qualquer formato de imagem futuro) — nunca configurada peça a peça.

- **Logo:** upload simples (mesmo componente de Dropzone já usado em KB-2/MEDIA-2), formatos de imagem comuns (PNG/JPG/SVG), sem obrigatoriedade — marca sem logo cadastrado gera conteúdo normalmente, com o layout se adaptando automaticamente para nunca deixar um espaço vazio (arch. §14.8).
- **Cor primária** e **cor secundária:** 2 campos (Seletor de Cor, §4.9), ambos opcionais, aplicados automaticamente aos elementos gráficos de toda peça gerada depois.
- **Fonte (opcional):** campo de texto livre com nome de fonte (Google Fonts) — sem obrigatoriedade; ausente, os templates usam uma fonte padrão do produto.
- **Estilo visual:** campo de texto livre, com sugestões clicáveis (moderno / elegante / minimalista / corporativo / jovem) — não uma lista fechada, o usuário pode descrever com as próprias palavras. Alimenta tanto os templates de composição quanto a seleção automática de voz (arch. §14.3), não é só decorativo.
...
5. **★ novo (preparação Missão 9) — Mecanismo de atualização de progresso durante a geração de vídeo (CAMP-4/5):** Supabase Realtime (o produto nunca usou até agora) ou polling simples (mais consistente com o padrão síncrono de todas as telas anteriores, mais barato de implementar)? Não bloqueia o início do código do pipeline em si (Fluxo 13), mas bloqueia a tela de acompanhamento — mesma decisão em aberto registrada em [flows.md, item 7](flows.md#decisões-em-aberto-fluxos).
6. **★ novo (preparação Missão 9) — Feedback de tempo estimado:** já que o pipeline de vídeo não tem duração determinística (depende de fornecedores externos), a tela mostra alguma estimativa ("normalmente leva de 2 a 5 minutos") ou só um indicador indeterminado de progresso? Não bloqueia o início do código.
