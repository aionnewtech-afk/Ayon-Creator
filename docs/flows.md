...
3. O n8n sequencia, via Provider Gateway (nunca lógica de fornecedor dentro do próprio workflow — [architecture.md §8](architecture.md#8-papel-do-n8n)): **narração** (Voice Provider = ElevenLabs — `synthesizeVoice`, gera áudio + `captionCues`, com fallback de estimativa de tempo defensivo — [architecture.md §3.5.1](architecture.md#351-geração-automática-de-vídeo-★-novo-preparação-missão-9)); **cenas** (Media Provider = Pexels — único caminho implementado na Etapa 1); e **composição final** (Video Render Provider = Shotstack — `composeVideo`, cenas + narração + legenda → MP4 vertical 9:16).
