...
> **Status:** v1.0 (revisão 38 — Sprint de Estabilização da Missão 12, achados reais em uso real)
> **Mudança desta revisão (38 — Sprint de Estabilização da Missão 12):** auditoria completa contra 9 problemas relatados em uso real (não achados de revisão de código) — causa raiz de cada um e correção, sem funcionalidade nova planejada. Mudanças que afetam esta arquitetura: (1) §14.6 — narração passa a resolver a peça primária ("Roteiro") **ao vivo** a cada chamada (`ContentPieceRepository.findPrimaryByCampaignId`, novo), nunca mais uma cópia pontual feita na aprovação da campanha (causa raiz do bug real "narra sem roteiro mesmo após inserido manualmente" — a cópia ficava desatualizada ou nunca acontecia se a geração do roteiro principal falhasse); falha explícita (`MissingScriptError`) antes de qualquer chamada à ElevenLabs sem roteiro válido. (2) §14.4/§14.7 — busca de foto ganha um `nicheOverride` opcional informado pelo usuário ao regenerar (ex. "praia"), sobrepondo a derivação automática por LLM quando preenchido, propagado até o workflow n8n. (3) Nova tela persistida "Campanhas" (`/campanhas`, `/campanhas/[id]`) — histórico real de todas as campanhas da marca; antes desta revisão não existia nenhum jeito de revisitar o conteúdo gerado depois do fluxo de criação (a revisão do pacote só existia como estado efêmero de componente cliente). (4) Retry automático (`fetchWithRetry`, novo, `packages/core/src/shared/fetch-with-retry.ts`) para os 4 providers com `fetch` cru (ElevenLabs/Pexels/Shotstack) e os 2 pontos de disparo do n8n — 3 tentativas, backoff exponencial; Anthropic já coberto pelo SDK oficial. (5) Contexto temporal real (`buildTemporalContextBlock`, novo, `packages/core/src/shared/temporal-context.ts`) injetado em todo prompt do Intelligence Hub e do Trend Engine — corrige respostas com datas desatualizadas; Trend Engine ganha também exclusão contra as últimas 5 pesquisas da marca (`excludeTitles`), corrigindo tendências repetidas. (6) **Novo**: `detectBrandColorMismatch` (`packages/core/src/asset-engine/detect-brand-color-mismatch.ts`) — único item de funcionalidade nova desta revisão, autorizado explicitamente pelo dono do produto após o fechamento da auditoria. Analisa as cores predominantes reais da logo (via `sharp`, resolução 150×150 com `kernel: "nearest"` — achado real: o kernel padrão lavava toda a paleta para tons pasteis por blending de anti-aliasing) e compara com `primary_color_hex`/`secondary_color_hex` cadastrados (distância euclidiana em RGB, limiar 100) — **só detecta e alerta, nunca extrai/aplica cor automaticamente**; exibido como banner não bloqueante em Perfil da Marca (§14.1). Achado de auditoria (não corrigido nesta revisão, dado de conta, não defeito de pipeline): a identidade visual configurada para a conta real testada (Todo Canto) diverge da logo enviada — cor secundária sem relação com a paleta real, logo sem transparência (fundo sólido embutido no arquivo). Ver [CHANGELOG.md](../CHANGELOG.md) v0.13.1 e [docs/changelog.md](changelog.md) v2.32 para a lista completa.
> **Mudança desta revisão (37 — achado real durante a implementação da Missão 12):** §15.2 estava incompleta — dizia que "telas administrativas que leem entre organizações podem usar o client de sessão normal... para tudo que já passa por essas 3 funções [`is_org_member`/`is_org_admin`/`is_org_editor`]", mas as 3 tabelas novas desta missão (`platform_admins`, `admin_audit_logs`, `provider_call_logs`) **não usam nenhuma dessas 3 funções — não têm nenhuma policy de RLS para `authenticated`, nem de leitura** (mesmo padrão de `provider_configs`/`specialists`, mas essa exceção não estava documentada para as tabelas novas). Achado durante a validação real de `/admin` (Etapa "layout e navegação administrativa"): `getCurrentSession()` consultava `platform_admins` com o client de sessão, sempre retornando `null` — nenhum `platform_admin` conseguia entrar em `/admin` (redirect 307 permanente). Corrigido consultando `platform_admins` com client de service role dentro de `getCurrentSession()`; §15.2, §15.8 (linhas de Dashboard/Logs/Auditoria) atualizadas para refletir que leituras de `platform_admins`/`admin_audit_logs`/`provider_call_logs` exigem service role, mesmo para o próprio admin ler sua própria linha. **Achado adicional (mesma revisão, antes de implementar a tela Usuários):** `user_profiles` também não é coberta pela extensão de RLS — sua única policy de `select`/`update` é `user_id = auth.uid()` (nunca usou `is_org_member`/`is_org_admin`/`is_org_editor`, então não recebeu o bypass de `platform_admin`). Um `platform_admin` não consegue ler/editar o perfil de outro usuário pelo client de sessão — a tela Usuários (§15.8) precisa de service role para `user_profiles`, além da já esperada necessidade de service role para a Admin API do Supabase (`auth.admin.*`, último login/redefinição de senha).
...
**★ Exceção — as 3 tabelas novas desta missão não têm nenhuma policy, nem de leitura (achado real, revisão 37):** `platform_admins`, `admin_audit_logs` e `provider_call_logs` têm RLS habilitado mas **zero policies** para `authenticated`/`anon` — mesmo padrão de `provider_configs`/`specialists` (§5.1), mas sem passar por `is_org_member` (não são tabelas por organização). Isso significa que a leitura dessas 3 tabelas — inclusive `getCurrentSession()` consultando o papel do próprio admin, e as telas Dashboard/Logs/Auditoria (§15.8) consultando as 3 — exige client de service role, nunca o client de sessão, mesmo dentro de uma rota já protegida por `requirePlatformAdmin()`.
...
| Usuários | `organization_members` via client de sessão; `user_profiles` via **service role** (achado real, revisão 37 — só tem policy `user_id = auth.uid()`, sem bypass de `platform_admin`); `auth.users` via Admin API do Supabase (service role, último login) | `organization_members` (papel/remoção, client de sessão); `user_profiles.status` (bloqueio, service role); reset de senha via Admin API do Supabase (service role) | qualquer `platform_admin` |
...
| Feedbacks | `user_feedback` via **service role** (§9.3 de database.md — sem policy de RLS para `authenticated`, mesmo padrão de `provider_configs`) | `user_feedback` via service role (arquivar, resposta interna, marcar como resolvido — ★ round 2, item 5, CRM interno, sem envio de resposta ao usuário) | qualquer `platform_admin` |
| Providers | `provider_call_logs` via **service role** (§15.7, achado real revisão 37), `provider_configs` via **service role** (sem policy de RLS para `authenticated`, §5.1) | `provider_configs` via service role (★ round 2, item 8 — ativar/desativar, trocar provider padrão da capability+tier, trocar credencial, colocar em manutenção; §15.11) — só `super_admin` | leitura: qualquer `platform_admin`; escrita: só `super_admin` |
| Logs | `pipeline_runs`, `credit_ledger` (pagamentos) via client de sessão; `admin_audit_logs`, `provider_call_logs` via **service role** (achado real, revisão 37) — visão unificada, filtrada | — (só leitura) | qualquer `platform_admin` |
| Branding | `brands` (§14.1) | `brands` (mesmos campos do Perfil da Marca, Missão 11, visão cross-organização) | qualquer `platform_admin` |
| Auditoria | `admin_audit_logs` via **service role** (§15.6, achado real revisão 37 — sem policy de RLS para `authenticated`) | — (só leitura) | qualquer `platform_admin` |
| Configurações | `provider_configs` via **service role** (sem policy de RLS para `authenticated`, §5.1); `credit_pricing`, `plans`, `feature_flags` via client de sessão (policy `select_authenticated`) | mesmas tabelas, todas via service role — primeira interface administrativa real para `provider_configs` (documentada desde a revisão 21 como "painel administrativo interno, não exposto ao cliente", §5.1, nunca implementada até agora) | só `super_admin` |
