...
> **Status:** v1.0 (revisão 13 — Intelligence Hub validado + Tool Registry registrado) — **aprovado, fonte oficial da verdade para a implementação**
...
3. ~~**Índice de retrieval da Knowledge Base:** uso de `pgvector` no Supabase Postgres — confirmar.~~ **Recomendação para a Missão 4 (revisão 13, aguardando confirmação do dono do produto):** adiar `pgvector`/embeddings para uma iteração futura. Motivo: nenhum provider de embedding está integrado hoje (a Anthropic não expõe API de embeddings; `PROVIDER_CAPABILITIES` só tem `llm`, `avatar`, `voice`, `media`, `trend_source`) — adicionar `pgvector` agora exigiria decidir e integrar um fornecedor novo (ex.: Voyage AI, recomendado pela própria Anthropic, ou OpenAI) antes de qualquer código de retrieval, o que atrasaria a Missão 4 sem necessidade clara ainda. Proposta: MVP da Knowledge Base usa retrieval por recência + `tags`/`source_type` (ver §3.2); a coluna `embedding` continua reservada em `knowledge_base_items` (nullable) para quando a decisão for tomada, sem migration destrutiva. **Esta é uma decisão de produto/infra, não só técnica — precisa de confirmação explícita antes do código da Missão 4.**
