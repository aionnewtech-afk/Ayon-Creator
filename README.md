...
[![CI](https://github.com/aionnewtech-afk/Ayon-Creator/actions/workflows/ci.yml/badge.svg)](https://github.com/aionnewtech-afk/Ayon-Creator/actions/workflows/ci.yml)
...
**Sprint 1**, **Missão 2 — "Conheça sua empresa"** (onboarding conversacional), **Missão 3 — Specialist Registry + primeiro Intelligence Hub funcional**, **Missão 4 — "Ensine sua Empresa para a IA"** (Knowledge Base), **Missão 5 — "O que está em Alta"** (Trend Engine), **Missão 6 — Billing** (planos Starter/Pro/Business, créditos, Mercado Pago — assinatura recorrente e créditos avulsos, portão de crédito obrigatório antes de qualquer geração), **Missão 7 — Asset Engine** (geração automática dos 5 formatos textuais do pacote de conteúdo via LLM Provider, upload manual dos 4 formatos visuais, edição/regeneração de peça, montagem automática do pacote final em `.zip`) e **Missão 8 — Learning Engine** ("O que Funcionou" / Brand Evolution — análise sob demanda dos sinais de aprovação/rejeição/edição de peça via Intelligence Hub, sugestões de ajuste sempre com decisão humana explícita, gratuita em todos os planos) implementadas e validadas ponta a ponta (Supabase + Anthropic + Mercado Pago reais). MVP concluído — projeto agora em **Sprint de Hardening rumo à v1.0** (auditoria completa em [docs/hardening-plan.md](docs/hardening-plan.md)); **Missão H1 — segurança crítica** (RLS de Storage, race conditions de provisionamento e portão de crédito, validação de upload, upgrade para Next.js 15.5.x) e **Missão H2 — fundação de qualidade** (Vitest para testes unitários/RLS/concorrência, Playwright para smoke test de browser com `LlmProvider` fake, CI no GitHub Actions) implementadas e validadas — ver [CHANGELOG.md](CHANGELOG.md) (`v0.9.0`). **Missão 9, Etapa 1 — geração automática de vídeo — concluída e validada em ambiente real** (Supabase + ElevenLabs + Pexels + Shotstack + n8n reais, ver [docs/changelog.md](docs/changelog.md) `v2.20`, [CHANGELOG.md](CHANGELOG.md) `v0.10.0`): Asset Engine ganha vídeo vertical 9:16 via banco de vídeo licenciado — narração (ElevenLabs), cenas (Pexels), composição + legenda (Shotstack), pipeline assíncrono orquestrado por n8n (primeira ativação real dessa infraestrutura no produto — instância própria, isolada, provisionada via Docker em `n8n/`). **Etapa 2 (futura, recurso Premium)** — avatar de IA (HeyGen), adiada, sem bloquear a Etapa 1, sem código ainda. **Filosofia de produto consolidada** (Princípio do Consultor Permanente — ver [PRD.md §1.1](PRD.md#11-princípio-do-consultor-permanente-★-novo-revisão-7)). Painel de especialistas + Coordinator AI resolvidos via Specialist Registry — nunca hardcoded. Todos os Core Engines do MVP (Brand Brain, Trend Engine, Intelligence Hub, Asset Engine, Learning Engine, Knowledge Base) implementados e validados. Documentação de referência para evolução dos especialistas em [docs/prompts/](docs/prompts/).

## Como usar

Guia completo do zero — subir o ambiente localmente, criar sua conta, assinar um
plano, criar sua primeira campanha e gerar seu primeiro vídeo automaticamente:
...
└── docs/
    ├── GETTING_STARTED.md   # guia de uso do zero — subir o ambiente, criar conta, gerar o primeiro vídeo
