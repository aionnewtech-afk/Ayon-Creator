...
> **Status do documento:** v1.0 (revisão 22 — Missão 9 dividida em 2 etapas) — **documentação aprovada pelo dono do produto; aguardando obtenção de contas/API keys (ElevenLabs, Pexels, Shotstack) antes do primeiro commit de código da Etapa 1**. Decisões residuais não relacionadas à filosofia seguem listadas em §13 e podem ser resolvidas durante o desenvolvimento, conforme necessidade.
